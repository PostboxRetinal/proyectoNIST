...
export class UserService {
   ...
   */
    ...
    // Asegúrate de que el role sea uno de los valores permitidos
    ...
    try {
      ...
      await updateProfile(user, {
        displayName: user.displayName || email.split('@')[0], // Usa el nombre de usuario del correo como displayName si no se ha configurado
      });

      // Guardar el rol en Firestore
      const userRef = doc(db, 'users', user.uid); // Referencia en Firestore
      try{
